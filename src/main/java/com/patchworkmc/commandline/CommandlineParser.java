...
import com.patchworkmc.commandline.types.EnumTypeMapper;
 ...
 *
 * @param <T> Type the {@link CommandlineParser#parse()} method uses for parsing and evaluating the
 *            commandline
 */
public class CommandlineParser<T> {
	 ...
	 * - If parseSucceeded == false && parseError != null => parse() has been called and the given
	 * commandline was bad
	...
	/**
	 * Creates a new CommandlineParser targeting an argument holder and for parsing the commandline
	 * given.
	 *
	 ...
	 * An exception is only thrown if a fatal error occurred while parsing. This means, if
	 * reflective access fails or any other unexpected exception happens.
	 * <p>
	 * This method does not throw if parsing itself fails. To check for such use
	 * {@link CommandlineParser#parseSucceeded()} and {@link CommandlineParser#getParseError()}. To
	 * generate a full help message use {@link CommandlineParser#generateHelpMessage(String, String, String, String, boolean, boolean)}
	 * String, String, boolean)} which will also include the reason parsing failed.
	 * </p>
	 *
	 * @return The object the options have been applied to, which is the object passed in the
	 * constructor
	...
	private void buildAccessors() throws CommandlineException {
		...
		for (Map.Entry<Integer, FieldAccessor> entry
			...
			if (entry.getKey() != index) {
				...
				// for example, if position = 1 and position = 3 is specified, we can't know what 2
				// would be. For the sake of simplicity don't allow this.
				throw new CommandlineException("Parameter index error, position gap? Expected " + entry.getKey() + " for parameter " + Arrays.toString(entry.getValue().names()) + " but got " + index);
			}

			if (index != 0 && !parameterAccessors.get(index - 1).required() && entry.getValue().required()) {
				// This might happen if a parameter which is required is specified to follow a
				// parameter which is optional. Since we can't know which parameter to fit the value
				// in, we don't allow this.
				throw new CommandlineException("Parameter " + Arrays.toString(entry.getValue().names()) + " which is " + "required follows optional parameter " + Arrays.toString(parameterAccessors.get(index - 1).names()));
			} else if (!entry.getValue().required() && remainingAccessor != null) {
				// Optional parameters with a remaining collector are also not allowed for the same
				// reason as above
	...
	private TypeMapper getTypeMapper(Class<? extends TypeMapper> annotatedClass, Field f) throws CommandlineException {
		if (annotatedClass != TypeMapper.NullTypeMapper.class) {
			...
			try {
				...
				MethodHandle constructor = METHOD_LOOKUP.findConstructor(annotatedClass, MethodType.methodType(void.class, Object.class, Field.class));
			...
			// Special case
			if (annotatedType.isEnum()) {
				// noinspection all
				return new EnumTypeMapper(annotatedType, argumentHolder, f);
			}
			...
			try {
				...
				throw new CommandlineException("Error while creating default type mapper for type " + annotatedType.getName(), t);
			}
		}
	}
	...
	private void processArgs() throws CommandlineException {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (currentArg.startsWith("--") || currentArg.startsWith("-")) {
				...
				for (FieldAccessor accessor : flagAccessors) {
					for (String accessorName : accessor.names()) {
						...
						if (accessorName.equals(name)) {
							// This does a bunch of checks to make a sure a flag requiring a value
							// gets one and a flag not requiring one does not get one passed
				...
				if (parameterIndex >= parameterAccessors.size()) {
	 ...
	 * {@link CommandlineParser#parse()} has been called and only if {@link
	 * CommandlineParser#parseSucceeded()} returned
	 * {@code false}.
	 *
	...
	/**
	 * Generates a nice help message which can be displayed to the user, optionally including
	 * colors.
	 *
	...
	public String generateHelpMessage(
		...
		if (!parameterAccessors.isEmpty()) {
			// Check how much space we require for the names of parameters so we can align their
			// descriptions
		...
		if (!flagAccessors.isEmpty()) {
			// Same as above for parameters, its just a bit more complex here since we have
			// separating commas
	...
	private void appendDescription(
		...
		for (int i = 0; i < descriptionParts.length; i++) {
			...
			if (i != 0) {
				...
				// We have a too long name, jump on the next line and then proceed as if it wouldn't
				// be the first line
				buffer.append("\n").append(repeat(' ', longestNameLength + 6));
			} else {
				// Calculate how much space we need to actually align the description right after
				// the name
